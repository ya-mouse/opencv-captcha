...
img_scale = cv2.bitwise_and(pre.img, pre.img, mask=closed) #closed)
closed[:3] = 0
closed[-3:] = 0
closed[:,:3] = 0
closed[:,-3:] = 0
...
cnts, img_dbg = detect_contours(img_scale, closed)
dump = False
if len(cnts) == 5 and dump:
    ...
    for r in cnts:
        ...
        cv2.imwrite('out/{}/{}-{}'.format(sys.argv[1][i], i, sys.argv[1]), closed[r.y:r.hy, r.x:r.wx])
        i += 1
